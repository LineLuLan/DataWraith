...
async def test_tui_composes_phase1_cards() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert pilot.app.query_one("#migration-command", Static) is not None
        assert pilot.app.query_one("#ai-analyze-command", Static) is not None
        assert pilot.app.query_one("#security-command", Static) is not None
        assert pilot.app.query_one("#report-export-command", Static) is not None
